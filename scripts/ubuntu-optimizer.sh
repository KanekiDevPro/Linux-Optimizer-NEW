...
swap_entry_active() {
    # Exact match on the DEVICE FIELD ($1): a substring grep would treat
    # /swapfile as "active" because of an unrelated /swapfile2 entry (and
    # the fstab cleanup below would DELETE that unrelated line).
...
swap_maker() {
    ...
    if [ "${SWAP_REPLACE:-0}" = "1" ]; then
        ...
        _fstab_prev=$(awk -v p="$SWAP_PATH" '$0 !~ /^[[:space:]]*(#|$)/ && $1 == p {print; exit}' /etc/fstab 2>/dev/null || true)
    ...
    # NOTE: the old swap file is removed only AFTER the fstab cleanup below
    # succeeds - deleting it first would leave fstab pointing at a missing
    # file when a later staging step fails.
    if swap_entry_active "$SWAP_PATH"; then
        ...
        if ! awk -v p="$SWAP_PATH" '($0 ~ /^[[:space:]]*(#|$)/) {print; next} $1 != p {print}' /etc/fstab > "$_fstab_tmp"; then
    ...
    # fstab cleanup succeeded - now drop the old swap file itself.
    if [ -f "$SWAP_PATH" ]; then
...
sysctl_optimizations() {
    local profile_input="${1:-${OPT_PROFILE:-}}"
    local profile=""
...
net.ipv4.conf.default.accept_source_route = 0"
    ...
    # Stage-then-replace: build in a same-directory temp file and mv into
    # place atomically (a truncating cat > breaks the header invariant).
    local sysctl_tmp
    sysctl_tmp=$(mktemp "$(dirname "$SYS_OPTIMIZER_PATH")/.99-optimizer.XXXXXX") || return 1
    OPT_TMPFILES+=("$sysctl_tmp")
    case "$selected_profile" in
        balanced)
            cat > "$sysctl_tmp" <<EOF
################################################################
...
EOF
    ...
    if ! mv -f "$sysctl_tmp" "$SYS_OPTIMIZER_PATH"; then
        red_msg "Failed to install $SYS_OPTIMIZER_PATH atomically."
        rm -f "$sysctl_tmp"
        return 1
    fi
    ...
    # mktemp here (NOT via $(new_tmp)): command substitution runs in a
    # subshell, so the OPT_TMPFILES append would be lost and the log
    # would never be cleaned up.
    apply_log=$(mktemp) || return 1
    OPT_TMPFILES+=("$apply_log")
    ...
    apply_log2=$(mktemp) || return 1
    OPT_TMPFILES+=("$apply_log2")
...
remove_old_ssh_conf() {
    ...
    local _ssh_tmp
    _ssh_tmp=$(mktemp "$(dirname "$SSH_PATH")/.sshd_config.XXXXXX") || return 1
    OPT_TMPFILES+=("$_ssh_tmp")
    if [ -n "$_match_at" ]; then
        cat "$_head" "$_tail" > "$_ssh_tmp" || return 1
    else
        cat "$_head" > "$_ssh_tmp" || return 1
    fi
    chmod --reference="$SSH_PATH" "$_ssh_tmp" 2>/dev/null || chmod 644 "$_ssh_tmp"
    chown --reference="$SSH_PATH" "$_ssh_tmp" 2>/dev/null || true
    if ! mv -f "$_ssh_tmp" "$SSH_PATH"; then
        red_msg "Failed to replace $SSH_PATH atomically; original left untouched."
        rm -f "$_ssh_tmp"
        return 1
    fi
...
update_sshd_conf() {
    ...
    if [ "$SSH_USE_DROPIN" = "1" ]; then
        ...
        set_sshd_opt() {
            ...
            local _out
            _out=$(mktemp "$(dirname "$SSH_PATH")/.sshd_config.XXXXXX") || { rm -f "$_g"; return 1; }
            OPT_TMPFILES+=("$_out")
            if ! cat "$_g" "$_t" > "$_out"; then
                rm -f "$_out"
                return 1
            fi
            chmod --reference="$SSH_PATH" "$_out" 2>/dev/null || chmod 644 "$_out"
            chown --reference="$SSH_PATH" "$_out" 2>/dev/null || true
            if ! mv -f "$_out" "$SSH_PATH"; then
                rm -f "$_out"
                return 1
            fi
            return 0
