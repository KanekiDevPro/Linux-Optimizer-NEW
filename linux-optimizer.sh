...
case "$OPT_OPTIMIZER_REF" in
    ''|*[^A-Za-z0-9._/-]*)
        printf '[*] ----- Invalid --optimizer-ref value: %s\n' "$OPT_OPTIMIZER_REF" >&2
        exit 64
        ;;
esac
...
# Name of the record this script owns inside the resolvconf database.
# The lo.* prefix decides priority via /etc/resolvconf/interface-order,
# where lo.* ranks above DHCP-supplied entries.
RESOLVCONF_IFACE="lo.linuxoptimizer"
...
print_selection_summary() {
    ...
    plain_msg "  $METHOD"
    plain_msg "Routing:"
    plain_msg "  Full routing is enforced: all lookups go to the servers above."
    plain_msg "  On corporate VPNs or Kubernetes clusters this may override split-DNS."
    plain_msg ""
}
...
write_resolvconf_direct() {
    ...
    tmp=$(mktemp /etc/.resolv.conf.linux-optimizer.XXXXXX) || {
        red_msg "Failed to stage a new /etc/resolv.conf."
        return 1
    }
...
# Deletes the resolvconf database record owned by this script. Must run
# before any file/symlink restoration, because resolvconf regenerates
# /etc/resolv.conf from its database and would otherwise re-add our servers.
rollback_resolvconf_record() {
    command -v resolvconf >/dev/null 2>&1 || return 0
    resolvconf -d "$RESOLVCONF_IFACE" >/dev/null 2>&1 || true
    return 0
}
...
rollback_dns() {
    ...
    # 0) resolvconf database entry (it outranks any file restored to disk)
    rollback_resolvconf_record
    ...
    if [ "${#NM_CHANGES[@]}" -gt 0 ]; then
        for item in "${NM_CHANGES[@]}"; do
            ...
            nmcli device reapply "$dev" >/dev/null 2>&1 || nmcli connection up "$uuid" >/dev/null 2>&1 || true
        done
        NM_CHANGES=()
    fi
